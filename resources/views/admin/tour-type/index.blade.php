...
<style>
    ...
    #tourTypesTable tbody tr:hover { background: #f0f3ff; }
    #tourTypesTable td { border-color: #eceef8; padding: .75rem .85rem; font-size: .88rem; color: #2d3050; }
    ...
    .btn-add-tour-type:hover { opacity: .9; transform: translateY(-1px); color: #fff; }
...
<div class="card border-0 shadow-sm" style="border-radius:16px; overflow:hidden;">
    ...
    <div class="card-header d-flex justify-content-between align-items-center py-3 px-4"
        ...
        <div>
            <h5 class="mb-0 fw-bold" style="color:#1a1a2e;">Tour Types</h5>
            <small class="text-muted">Manage all tour types</small>
        </div>
        <button class="btn-add-tour-type" data-bs-toggle="modal" data-bs-target="#createModal">
    ...
    <div class="card-body p-0">
        <div class="table-responsive">
            <table class="table mb-0" id="tourTypesTable">
                <thead>
                    <tr>
                        <th>#</th>
                        <th>Image</th>
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td>
                            @if($tourType->image)
                                <img src="{{ asset('uploads/tour-types/'.$tourType->image) }}" class="tour-type-thumb" alt="{{ $tourType->name }}">
                            @else
                                <img src="{{ asset('contents/admin/images/no-image.png') }}" class="tour-type-thumb" alt="No Image">
                            @endif
                        </td>
                        <td>
                            <div class="fw-semibold">{{ $tourType->name }}</div>
                            <div class="slug-text">/{{ $tourType->slug }}</div>
                        </td>
                        <td>
                            @if($tourType->icon)
                                <i class="{{ $tourType->icon }} icon-display"></i>
                            @else
                                <span class="text-muted">—</span>
                            @endif
                        </td>
                        <td>{{ $tourType->sort_order ?? '—' }}</td>
                        <td>
                            @if($tourType->status == 1)
                        ...
                        <td>
                            <div class="actions-wrap">
                                ...
                                {{-- Delete --}}
                                <form action="{{ route('admin.tour-types.destroy', $tourType->id) }}"
                                      method="POST"
        ...
        <div class="modal-content">
            ...
            <div class="modal-header">
                <h5 class="modal-title" id="createModalLabel">
                    <i class="fas fa-plus"></i> Add New Tour Type
                </h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>

            <form method="POST" action="{{ route('admin.tour-types.store') }}" enctype="multipart/form-data">
                @csrf
                ...
                <div class="modal-body">
                    ...
                    <div class="modal-section">
                        ...
                        <div class="row g-3">
                            ...
                            <div class="col-12">
                                ...
                                <input type="text" name="name" class="form-control" value="{{ old('name') }}" placeholder="e.g. Adventure, Beach, Cultural">
                                @error('name')<small class="text-danger">{{ $message }}</small>@enderror
                            </div>

                            <div class="col-12">
                                <label>Icon (Font Awesome Class)</label>
                                <input type="text" name="icon" class="form-control" value="{{ old('icon') }}" placeholder="e.g. fa-solid fa-mountain">
                                <small class="text-muted">Enter Font Awesome icon class (e.g. fa-solid fa-umbrella-beach)</small>
                    ...
                    <div class="modal-section">
                        <div class="modal-section-title"><i class="fas fa-image"></i> Image</div>
                    ...
                    <div class="modal-section">
                        <div class="modal-section-title"><i class="fas fa-align-left"></i> Description</div>
                        <div class="row g-3">
                            <div class="col-12">
                                <label>Short Description</label>
                                <textarea name="short_description" rows="3" class="form-control" placeholder="Brief description about this tour type...">{{ old('short_description') }}</textarea>
                            </div>
                        </div>
                    </div>
                ...
                <div class="modal-footer">
                    ...
                    <button type="submit" class="btn-submit">
                        <i class="fas fa-save"></i> Save Tour Type
                    </button>
                </div>
        ...
        <div class="modal-content">
            ...
            <div class="modal-header">
                <h5 class="modal-title">
                    <i class="fas fa-pencil-alt"></i> Edit Tour Type
                </h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            ...
            <form id="editForm" method="POST" enctype="multipart/form-data">
                ...
                <div class="modal-footer">
                    <button type="button" class="btn-cancel" data-bs-dismiss="modal">Cancel</button>
                    <button type="submit" class="btn-submit">
                        <i class="fas fa-save"></i> Update Tour Type
                    </button>
                </div>
...
<div class="modal fade tour-modal" id="viewModal" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <div class="modal-header">
                <h5 class="modal-title">
                    <i class="fas fa-eye"></i> Tour Type Details
                </h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
...
function openEditModal(tourTypeId) {
    ...
    fetch(`/admin/tour-types/${tourTypeId}/modal-data`)
        .then(response => {
            if (!response.ok) {
        ...
        .then(data => {
            ...
            form.action = `/admin/tour-types/${tourType.id}/update`;

            const currentImg = tourType.image
                ? `<div class="img-preview-wrap">
                     <img src="/uploads/tour-types/${tourType.image}" alt="current">
            ...
            <div class="modal-section">
                ...
                <div class="row g-3">
                    <div class="col-12">
                        <label>Tour Type Name *</label>
                        <input type="text" name="name" class="form-control" value="${escHtml(tourType.name)}">
                    </div>
                    <div class="col-12">
                        <label>Icon (Font Awesome Class)</label>
                        <input type="text" name="icon" class="form-control" value="${escHtml(tourType.icon ?? '')}" placeholder="e.g. fa-solid fa-mountain">
                        <small class="text-muted">Enter Font Awesome icon class (e.g. fa-solid fa-umbrella-beach)</small>
                    </div>
                    <div class="col-md-6">
                        <label>Sort Order</label>
                        <input type="number" name="sort_order" class="form-control" value="${tourType.sort_order ?? 0}">
                    </div>
                    <div class="col-md-6">
                        <label>Status</label>
                        <select name="status" class="form-control">
                            <option value="1" ${tourType.status == 1 ? 'selected' : ''}>Active</option>
                            <option value="0" ${tourType.status == 0 ? 'selected' : ''}>Inactive</option>
                        </select>
                    </div>
                </div>
            </div>
            ...
            <div class="modal-section">
                ...
                <div class="row g-3">
                    <div class="col-12">
                        <label>Short Description</label>
                        <textarea name="short_description" rows="3" class="form-control">${escHtml(tourType.short_description ?? '')}</textarea>
                    </div>
                </div>
            </div>`;
        })
        .catch((error) => {
            ...
            body.innerHTML = `<div class="alert alert-danger m-0">Failed to load tour type data. Please try again.</div>`;
        });
}
...
function openViewModal(tourTypeId) {
    ...
    fetch(`/admin/tour-types/${tourTypeId}/modal-data`)
        ...
        .then(data => {
            const t = data.tourType;

            const imgSrc = t.image
                ? `/uploads/tour-types/${t.image}`
            ...
            const iconDisplay = t.icon
                ? `<i class="${t.icon}" style="font-size: 1.8rem; color: #0f3460;"></i>`
            ...
            body.innerHTML = `
            <div class="view-hero">
                <img src="${imgSrc}" alt="${escHtml(t.name)}">
                <div class="view-hero-overlay"></div>
                <div class="view-hero-title">${escHtml(t.name)}</div>
            </div>

            <div class="view-info-grid">
                <div class="view-info-item">
                    <div class="view-info-label">Icon</div>
                    <div class="view-info-value">${iconDisplay}</div>
                </div>
                <div class="view-info-item">
                    <div class="view-info-label">Slug</div>
                    <div class="view-info-value" style="font-size:.8rem;font-weight:400;">${escHtml(t.slug)}</div>
                </div>
                <div class="view-info-item">
                    <div class="view-info-label">Sort Order</div>
                    <div class="view-info-value">${t.sort_order ?? '—'}</div>
                </div>
                <div class="view-info-item">
                    <div class="view-info-label">Status</div>
                    <div class="view-info-value">${statusBadge}</div>
                </div>
            </div>
