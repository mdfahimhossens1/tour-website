...
<style>
  ...
  .um-table tbody tr:hover .um-avatar {
    transform: scale(1.1);
  }
  ...
  .um-modal-overlay.open .um-modal {
  ...
  .um-field input[type="file"] {
    padding: 8px 12px;
    cursor: pointer;
  }

  .um-field .preview-wrap {
    margin-top: 10px;
  }

  .um-field .preview-wrap img {
    width: 60px;
    height: 60px;
    border-radius: 50%;
    object-fit: cover;
  ...
  .um-toast-danger .um-toast-icon {
    background: rgba(239,68,68,.15);
    color: var(--um-danger);
  }

  .um-toast-info .um-toast-icon {
    background: rgba(56,189,248,.15);
    color: var(--um-info);
  }
  ...
  /* ── scrollbar ── */
  .um-modal::-webkit-scrollbar {
    width: 5px;
  }

  .um-modal::-webkit-scrollbar-track {
    background: transparent;
  }

  .um-modal::-webkit-scrollbar-thumb {
    background: var(--um-border);
    border-radius: 10px;
  }
...
<div class="um-wrap">
      ...
      <div>
        ...
        <div class="d-flex gap-2 mt-3 flex-wrap">
          ...
          <span class="stat-pill">
            <span class="dot" style="background:var(--um-success)"></span>
            {{ $users->where('status',1)->count() }} Active
          </span>

          <span class="stat-pill">
            <span class="dot" style="background:var(--um-danger)"></span>
            {{ $users->where('status',0)->count() }} Inactive
          </span>

          <span class="stat-pill">
            <span class="dot" style="background:var(--um-accent2)"></span>
            {{ $users->count() }} Total
          </span>
  ...
  <div class="um-table-card">
    ...
    <div class="search-bar justify-content-between flex-wrap gap-2">
      ...
      <span
        style="font-size:.8rem;color:var(--um-muted);"
        id="um-count"
      ></span>
...
@php
    ...
    $rk = str_replace(
        [' ','-'],
        '_',
        strtolower(trim(optional($user->role)->role_name ?? 'user'))
    );
...
@endphp
          ...
          <tr
            data-search="{{ strtolower($user->name . ' ' . $user->email . ' ' . $user->username . ' ' . ($user->role->role_name ?? '')) }}"
          >

            <td style="color:var(--um-muted);font-size:.8rem;">
              {{ $i+1 }}
            </td>
            ...
            <td>
              ...
              <div class="d-flex align-items-center gap-3">
                ...
                @else
                  ...
                  <img
                    src="{{ asset('contents/admin/images/avatar.jpg') }}"
                    class="um-avatar"
                    alt="{{ $user->name }}"
                  >
                ...
                <div>
                  ...
                  <div style="font-size:.78rem;color:var(--um-muted);">
                    {{ $user->email }}
                  </div>
            ...
            <td style="color:var(--um-muted);font-size:.85rem;">
              {{ $user->phone ?? '—' }}
            </td>
            ...
            <td>
              ...
              <div class="um-actions-cell">
                ...
                <button
                  class="um-btn um-btn-danger-ghost"
                  title="Delete"
      ...
      <div class="um-modal-body">
        ...
        <div class="um-grid-2">
          ...
          <div class="um-field">
            ...
            <label>
              Full Name
              <span style="color:var(--um-danger)">*</span>
            </label>
        ...
        <div class="um-grid-2">
          ...
          <div class="um-field">
            ...
            <select
              name="role_id"
              id="add_role_id"
              onchange="toggleVendorCommission(this)"
            >
...
@endforeach
          ...
          <div class="um-field">
            ...
            <input
              ...
              name="photo"
              accept="image/*"
              onchange="previewImg(this,'add-preview')"
            >
        ...
        >
          ...
          <input
            ...
            value="{{ old('commission_rate', '10.00') }}"
            min="0"
            max="100"
            step="0.01"
            placeholder="10.00"
          ...
          <div
            ...
            "
          >
            This commission percentage will be applied to this vendor's bookings.
          </div>
      ...
      <div class="um-modal-body">
        ...
        <div class="um-field">
          ...
          <input
            ...
            name="photo"
            accept="image/*"
            onchange="previewImg(this,'edit-preview')"
          >
...
<div class="um-modal-overlay" id="deleteModal">
  ...
  >
    ...
    >
      ...
      <p
        style="color:var(--um-muted);font-size:.88rem;margin-bottom:4px;"
      >
        ...
        <strong
          id="delete-name"
          style="color:var(--um-text)"
        ></strong>.

      </p>

      <p
        style="color:var(--um-muted);font-size:.8rem;"
      >

        This action

        <strong style="color:var(--um-danger)">
          cannot be undone
        </strong>.
    ...
    >
      ...
      >
        ...
        <button
          type="submit"
          class="um-btn um-btn-primary"
          style="background:var(--um-danger);"
        >
...
document.addEventListener('keydown', e => {
  ...
  if(e.key === 'Escape') {
    ...
    document
      .querySelectorAll('.um-modal-overlay.open')
...
function openAddModal() {
  ...
  // Check role again when modal opens
  var roleSelect = document.getElementById('add_role_id');

  if(roleSelect) {

    toggleVendorCommission(roleSelect);
...
// ── Vendor Commission Toggle ──────────────────────

function toggleVendorCommission(select) {
  ...
  var commissionField =
    document.getElementById('vendorCommissionField');

  var commissionInput =
    document.getElementById('vendor_commission_rate');
...
function openEditModal(u) {
  ...
  form.action =
    "{{ url('admin/users/update') }}/" + u.slug;


  @if($isSuperAdmin)

    var sel =
      document.getElementById('edit_role_id');
...
(function() {
  ...
  var inp =
    document.getElementById('um-search');

  var rows =
    document.querySelectorAll(
      '#um-tbody tr[data-search]'
    );

  var count =
    document.getElementById('um-count');
...
function showToast(type, title, msg) {
  ...
  t.innerHTML = `
    ...
    <div>
      ...
      <div class="um-toast-title">
        ${title}
      </div>

      <div class="um-toast-msg">
        ${msg}
      </div>
