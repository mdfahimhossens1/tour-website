...
.b-header::before {
  ...
  background:url("data:image/svg+xml,%3Csvg width='60' height='60' viewBox='0 0 60 60' xmlns='http://www.w3.org/2000/svg'%3E%3Cg fill='%23f59e0b' fill-opacity='0.05'%3E%3Cpath d='M30 0L60 30L30 60L0 30Z'/%3E%3C/g%3E%3C/svg%3E");
}
.b-header::after {
...
.b-search-input:focus { border-color:var(--b-accent); box-shadow:0 0 0 3px rgba(245,158,11,.12); }
...
.b-table tbody tr:hover { background:rgba(245,158,11,.04); }
...
.b-user-avatar-placeholder { width:34px; height:34px; border-radius:50%; background:var(--b-accent); display:flex; align-items:center; justify-content:center; font-size:.75rem; font-weight:700; color:#451a03; flex-shrink:0; }
...
.b-amount { font-family:'JetBrains Mono',monospace; font-weight:600; color:var(--b-success); }
...
.b-badge-confirmed { background:rgba(34,197,94,.12);  color:#86efac; border:1px solid rgba(34,197,94,.25); }
.b-badge-cancelled { background:rgba(239,68,68,.12);  color:#fca5a5; border:1px solid rgba(239,68,68,.25); }
.b-badge-completed { background:rgba(167,139,250,.12);color:#c4b5fd; border:1px solid rgba(167,139,250,.25); }
...
.b-btn-primary { background:var(--b-accent); color:#451a03; padding:9px 18px; font-size:.85rem; }
.b-btn-primary:hover { background:var(--b-accent2); transform:translateY(-1px); color:#451a03; }
...
.b-btn-success-ghost { background:rgba(34,197,94,.1); color:#86efac; border:1px solid rgba(34,197,94,.2); padding:6px 10px; font-size:.78rem; border-radius:6px; }
.b-btn-success-ghost:hover { background:rgba(34,197,94,.2); }
...
<div class="b-wrap">
  ...
  <div class="b-header">
    <div class="d-flex justify-content-between align-items-center flex-wrap gap-3">
      <div>
        <div class="title"><i class="fas fa-hourglass-half me-2"></i>Pending Bookings</div>
        <div class="subtitle">Review and confirm new booking requests</div>
        <div class="d-flex gap-2 mt-3 flex-wrap">
          <span class="stat-pill"><span class="dot" style="background:var(--b-accent)"></span>{{ $bookings->count() }} Pending</span>
          <span class="stat-pill"><span class="dot" style="background:var(--b-success)"></span>৳{{ number_format($bookings->sum('total_amount'), 0) }} Total Value</span>
        </div>
      </div>
      <div style="position:relative;z-index:1;">
        <a href="{{ route('admin.bookings.confirmed') }}" class="b-btn b-btn-outline">
  ...
  <div class="b-table-card">
    <div class="b-search-bar">
      <div class="b-search-wrap">
        ...
        <input type="text" class="b-search-input" id="b-search" placeholder="Search by code, user or tour...">
      </div>
    ...
    <div style="overflow-x:auto;">
      <table class="b-table">
        ...
        <tbody id="b-tbody">
          ...
          <tr data-search="{{ strtolower($booking->booking_code.' '.($booking->user->name ?? '').' '.($booking->tour->title ?? '')) }}">
            ...
            <td>
              <div class="b-tour-name" title="{{ $booking->tour->title ?? 'N/A' }}">{{ $booking->tour->title ?? 'N/A' }}</div>
            ...
            <td>
              <div class="b-actions-cell">
                <a href="{{ route('admin.bookings.show', $booking->id) }}" class="b-btn b-btn-icon" title="View Details">
                  <i class="fas fa-eye"></i>
                </a>
                <button class="b-btn b-btn-success-ghost" title="Confirm"
  ...
  <div class="b-modal">
    <div class="b-modal-body">
      ...
      <p style="color:var(--b-muted);font-size:.88rem;">
        Booking <strong id="confirm-code" style="color:var(--b-text)"></strong> will be confirmed.
      </p>
    </div>
    <div class="b-modal-footer">
      ...
      <form id="confirmForm" method="POST" style="display:inline;">
        @csrf
        <button type="submit" class="b-btn b-btn-primary" style="background:var(--b-success);color:#0f2027;">
...
(function(){
  ...
  cnt.textContent = rows.length + ' of ' + rows.length + ' bookings';
})();
